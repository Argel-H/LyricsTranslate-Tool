...
export function AudioPlayerBar({
      ...
      if (audioRef.current) {
        ...
        if (activeKey) {
          if (activeKey === stableKeyRef.current) {
            ...
            if (stableCountRef.current >= STABILITY_THRESHOLD && activeKey !== lastActiveKeyRef.current) {
  ...
  return (
    <div className="flex-1 flex items-center gap-3 relative">
          ...
          <>
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-72 bg-surface-container border border-outline-variant/20 rounded-2xl shadow-xl p-4 flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-on-surface-variant/50">{t("common.or")}</span>
                <div className="flex-1 border-t border-outline-variant/10" />
              </div>
      ...
      {/* Source chip */}
      {!audioSrc ? (
        <span className="text-[10px] font-medium text-on-surface-variant bg-surface-container-highest px-2 py-0.5 rounded-full shrink-0">{t("player.sourceNone")}</span>
      ) : audioSrc.startsWith("blob:") ? (
        <span className="text-[10px] font-medium text-green-400 bg-green-400/10 px-2 py-0.5 rounded-full shrink-0">{t("player.sourceLocal")}</span>
      ) : (
        <span className="text-[10px] font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-full shrink-0">{t("player.sourceUrl")}</span>
      )}
