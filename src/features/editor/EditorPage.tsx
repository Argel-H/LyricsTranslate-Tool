...
import { Edit, Save, Sparkles, Loader2, Plus, Download, FileText } from "lucide-react";
...
export function EditorPage() {
  ...
  const handleAddLine = async () => {
    ...
    const lastEnd = keys.length > 0
      ? parseTime(lyrics[keys[keys.length - 1]!]!.time_end)
      : 0;
  ...
  const handleTimeAdjust = (key: string, field: "time_start" | "time_end", direction: 1 | -1) => {
    if (!currentProject) return;
    const lyrics = currentProject.lyrics;
    const keys = Object.keys(lyrics);
    ...
    if (field === "time_start") {
      ...
      const max = idx < keys.length - 1 ? parseTime(lyrics[keys[idx + 1]!]!.time_start) : Infinity;
      if (newValue < min || newValue > max) return;
  ...
  const handleDownload = (format: "lrc" | "srt", useTranslation: boolean) => {
    const content = format === "lrc" ? generateLRC(useTranslation) : generateSRT(useTranslation);
    ...
    >
      <MasterCard bgColor="bg-surface-container-lowest">
        <div className="max-w-[1400px] mx-auto w-full flex flex-col gap-lg pb-32">
          ...
          <div ref={tableRef} className="bg-surface-container-low rounded-[32px] overflow-hidden flex flex-col shadow-lg border border-outline-variant/10">
                ...
                return (
                  <TableRow
                    ...
                    translationPlaceholder={
                      !line.translation ? t("editor.translatePlaceholder") : undefined
                    }
                    state={state}
                    ...
                    onTimeStartAdd={() => handleTimeAdjust(key, "time_start", 1)}
                    onTimeStartRemove={() => handleTimeAdjust(key, "time_start", -1)}
                    onTimeEndAdd={() => handleTimeAdjust(key, "time_end", 1)}
                    onTimeEndRemove={() => handleTimeAdjust(key, "time_end", -1)}
